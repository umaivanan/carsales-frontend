...
//         "http://localhost:8000/api/auth/register", // Replace with your backend API URL
//         {
//           name,
//           email,
//           password,
...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Optionally navigate to the first page after successful registration
      setTimeout(() => {
        navigate("/firstpage"); // Navigate to FirstPage.js after success
      }, 2000);
    } catch (err) {
      console.error(err.response?.data?.message || "Registration failed");
      setError(err.response?.data?.message || "Something went wrong");
  ...
  useEffect(() => {
    // If the registration is successful, navigate to the first page
    if (success) {
      setTimeout(() => {
        navigate("/firstpage"); // After successful registration, go to FirstPage.js
      }, 2000);
    }
  }, [success, navigate]); // Only runs if success is updated
